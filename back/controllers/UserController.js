...
class UserController{
    async register(req, res,next){
        try {
            const { name, surname, email, password, phone, profile, image_path } = req.body;
    ...
    async findPeople(req, res,next){ //разобраться с этим чуть позже понять как оптимизировать
        try {
    ...
    async updateUser(req, res,next){ //тут тоже нужно пересмотреть немного - возможно удасться оптимизировать с помощью клиента
        try {
            const { userID } = req.params;
            const { name, surname, email, phone, profile, image_path, password } = req.body;
    ...
    async getAllUsersAdmin(req, res,next){ //позже добавить функционал, пока тоже самое что и у обычного юзера
        try {
            const users = await User.findAll();
            return res.json({ users });
        } catch (error) {
    ...
    async upgradeRole(req, res,next){
        try {
            ...
            if (adminPassword !== user.admin_password) {
                return next(ApiError.unauthorized('Incorrect admin password'));
            }
